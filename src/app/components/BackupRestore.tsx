...
function BackupStatus({ enabled }: BackupStatusProps) {
  ...
  return (
    <Box as="span" gap="100" alignItems="Center">
      ...
      >
        {enabled ? t('sharedUi.backupRestore.connected') : t('sharedUi.backupRestore.disconnected')}
      </Text>
    </Box>
  );
}
  ...
  return (
    <Box as="span" gap="100" alignItems="Center">
      ...
      <Text as="span" size="L400" style={{ color: color.Primary.Main }}>
        {t('sharedUi.backupRestore.syncing', { count })}
      </Text>
    </Box>
  );
}
  ...
  return (
    <Box grow="Yes" gap="200" alignItems="Center">
      <Badge variant="Secondary" fill="Solid" radii="300">
        <Text size="L400">{t('sharedUi.backupRestore.restoring0')}</Text>
      </Badge>
...
function BackupProgress({ total, downloaded }: BackupProgressProps) {
  ...
  return (
    <Box grow="Yes" gap="200" alignItems="Center">
      <Badge variant="Secondary" fill="Solid" radii="300">
        <Text size="L400">
          {t('sharedUi.backupRestore.restoring', {
    ...
    <Box direction="Column">
      {trust.matchesDecryptionKey ? (
        <Text size="T200" style={{ color: color.Success.Main }}>
          <b>{t('sharedUi.backupRestore.backupHasTrustedDecryptionKey')}</b>
        </Text>
      ) : (
        <Text size="T200" style={{ color: color.Critical.Main }}>
          <b>{t('sharedUi.backupRestore.backupDoesNotHaveTrustedDecryptionKey')}</b>
        </Text>
      )}
      {trust.trusted ? (
        <Text size="T200" style={{ color: color.Success.Main }}>
          <b>{t('sharedUi.backupRestore.backupHasTrustedBySignature')}</b>
        </Text>
      ) : (
        <Text size="T200" style={{ color: color.Critical.Main }}>
          <b>{t('sharedUi.backupRestore.backupDoesNotHaveTrustedSignature')}</b>
        </Text>
      )}
    </Box>
  );
...
export function BackupRestoreTile({ crypto }: BackupRestoreTileProps) {
  ...
  return (
    <InfoCard
      variant="Surface"
      title={t('sharedUi.backupRestore.encryptionBackup')}
      after={
                  ...
                  <Box direction="Column" gap="100">
                    <Box direction="Column" gap="200">
                      <InfoCard
                        ...
                        title={t('sharedUi.backupRestore.backupDetails')}
                        description={
                          <>
                            <span>
                              {t('sharedUi.backupRestore.version', {
                                version: backupInfo?.version ?? 'NIL',
                              })}
                            </span>
                            <br />
                            <span>
                              {t('sharedUi.backupRestore.keys', {
                                keyCount: backupInfo?.count ?? 'NIL',
                              })}
                            </span>
                          </>
                        }
                    ...
                    >
                      <Text size="B300">{t('sharedUi.backupRestore.restoreBackup')}</Text>
                    </Button>
                  </Box>
                </Menu>
              </FocusTrap>
    ...
    >
      ...
      {!backupEnabled && backupInfo === null && (
        <Text size="T200" style={{ color: color.Critical.Main }}>
          <b>{t('sharedUi.backupRestore.noBackupPresentOnServer')}</b>
        </Text>
      )}
