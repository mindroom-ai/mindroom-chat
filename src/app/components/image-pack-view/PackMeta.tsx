...
function ImagePackAvatar({ url, name }: ImagePackAvatarProps) {
  ...
  return (
    <Avatar size="500" className={ContainerColor({ variant: 'Secondary' })}>
      {url ? (
        <AvatarImage src={url} alt={name ?? t('sharedUi.packMeta.unknown')} />
      ) : (
        <AvatarFallback>
      ...
      <Box grow="Yes" direction="Column" gap="300">
        <Box direction="Column" gap="100">
          <Text className={BreakWord} size="H5">
            {meta.name ?? t('sharedUi.packMeta.unknown')}
          </Text>
          {meta.attribution && (
        ...
        {canEdit && (
          <Box gap="200">
            <Chip
              ...
              outlined
            >
              <Text size="B300">{t('sharedUi.packMeta.edit')}</Text>
            </Chip>
          </Box>
        )}
      </Box>
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="400">
      <Box gap="400">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('sharedUi.packMeta.packAvatar')}</Text>
          {uploadAtom ? (
            ...
            <Box gap="200">
              ...
              >
                <Text size="B300">{t('sharedUi.packMeta.upload')}</Text>
              </Button>
              {!avatar && meta.avatar && (
                <Button
                  type="button"
                ...
                >
                  <Text size="B300">{t('sharedUi.packMeta.reset')}</Text>
                </Button>
              )}
              {avatar && (
                <Button
                ...
                >
                  <Text size="B300">{t('sharedUi.packMeta.remove')}</Text>
                </Button>
              )}
            </Box>
          )}
      ...
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('sharedUi.packMeta.name')}</Text>
        <Input name="nameInput" defaultValue={meta.name} variant="Secondary" radii="300" required />
      </Box>
      <Box direction="Inherit" gap="100">
        <Text size="L400">{t('sharedUi.packMeta.attribution')}</Text>
        <TextArea
      ...
      <Box gap="300">
        <Button type="submit" variant="Success" size="300" radii="300" disabled={uploadingAvatar}>
          <Text size="B300">{t('sharedUi.packMeta.save')}</Text>
        </Button>
        <Button
          type="reset"
          onClick={onCancel}
          ...
          radii="300"
        >
          <Text size="B300">{t('sharedUi.packMeta.cancel')}</Text>
        </Button>
      </Box>
    </Box>
  );
