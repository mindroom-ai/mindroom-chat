...
export function RoomPublishedAddresses({ permissions }: RoomPublishedAddressesProps) {
  ...
  return (
    ...
    >
      <SettingTile
        title={t('featureUi.commonSettings.general.roomAddress.publishedAddresses')}
        description={
          <span>
            <Trans
              i18nKey="featureUi.commonSettings.general.roomAddress.publishedAddressesDescription"
              components={{ public: <b /> }}
            />
          </span>
        }
      ...
      <CutoutCard variant="Surface" style={{ padding: config.space.S300 }}>
        {publishedAliases.length === 0 ? (
          <Box direction="Column" gap="100">
            <Text size="L400">{t('featureUi.commonSettings.general.roomAddress.noAddresses')}</Text>
            <Text size="T200">
              {t('featureUi.commonSettings.general.roomAddress.publishRequiresLocalAddress')}
            </Text>
          </Box>
        ) : (
                ...
                <Box grow="Yes" gap="Inherit" alignItems="Center">
                  ...
                  {alias === canonicalAlias && (
                    <Badge variant="Success" fill="Solid" size="500">
                      <Text size="L400">
                        {t('featureUi.commonSettings.general.roomAddress.main')}
                      </Text>
                    </Badge>
                  )}
                </Box>
                {canEditCanonical && (
                  <Box shrink="No" gap="100">
                    {alias === canonicalAlias ? (
                      ...
                      >
                        <Text size="B300">
                          {t('featureUi.commonSettings.general.roomAddress.unsetMain')}
                        </Text>
                      </Chip>
                    ) : (
                      <Chip
                      ...
                      >
                        <Text size="B300">
                          {t('featureUi.commonSettings.general.roomAddress.setMain')}
                        </Text>
                      </Chip>
                    )}
                  </Box>
    ...
    <Box as="form" onSubmit={handleSubmit} direction="Column" gap="200">
      <Box gap="200">
        ...
        <Box shrink="No">
          ...
          >
            <Text size="B400">{t('featureUi.commonSettings.general.roomAddress.save')}</Text>
          </Button>
        </Box>
      </Box>
      {addState.status === AsyncStatus.Error && (
        <Text style={{ color: color.Critical.Main }} size="T200">
          {(addState.error as MatrixError).httpStatus === 409
            ? t('featureUi.commonSettings.general.roomAddress.addressAlreadyInUse')
    ...
    <Box direction="Column" gap="300">
      {selectedAliases.length > 0 && (
        <Box gap="200">
          <Box grow="Yes">
            <Text size="L400">
              {t('featureUi.commonSettings.general.roomAddress.selectedCount', {
          ...
          <Box shrink="No" gap="Inherit">
            {canEditCanonical &&
              (selectHasPublished ? (
                ...
                >
                  <Text size="B300">
                    {t('featureUi.commonSettings.general.roomAddress.unpublish')}
                  </Text>
                </Chip>
              ) : (
                <Chip
                ...
                >
                  <Text size="B300">
                    {t('featureUi.commonSettings.general.roomAddress.publish')}
                  </Text>
                </Chip>
              ))}
            <Chip
            ...
            >
              <Text size="B300">{t('featureUi.commonSettings.general.roomAddress.delete')}</Text>
            </Chip>
          </Box>
        </Box>
      )}
            ...
            <Box shrink="No" gap="100">
              {published && (
                <Badge variant="Success" fill="Soft" size="500">
                  <Text size="L400">
                    {t('featureUi.commonSettings.general.roomAddress.published')}
                  </Text>
                </Badge>
              )}
            </Box>
...
export function RoomLocalAddresses({ permissions }: { permissions: RoomPermissionsAPI }) {
  ...
  return (
    ...
    >
      <SettingTile
        title={t('featureUi.commonSettings.general.roomAddress.localAddresses')}
        description={t('featureUi.commonSettings.general.roomAddress.localAddressesDescription')}
        after={
          <Button
            type="button"
          ...
          >
            <Text as="span" size="B300" truncate>
              {expand
                ? t('featureUi.commonSettings.general.roomAddress.collapse')
                : t('featureUi.commonSettings.general.roomAddress.expand')}
            </Text>
          </Button>
        }
      />
        ...
        <CutoutCard variant="Surface" style={{ padding: config.space.S300 }}>
          {localAliasesState.status === AsyncStatus.Loading && (
            <Box gap="100">
              ...
              <Text size="T200">{t('featureUi.commonSettings.general.roomAddress.loading')}</Text>
            </Box>
          )}
          {localAliasesState.status === AsyncStatus.Success &&
            (localAliasesState.data.length === 0 ? (
              <Box direction="Column" gap="100">
                <Text size="L400">
                  {t('featureUi.commonSettings.general.roomAddress.noAddresses')}
                </Text>
              </Box>
            ) : (
              <LocalAddressesList
