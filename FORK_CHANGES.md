...
### CINNY-133 - Invite autocomplete menu portals past host clipping (2026-07-02)
...
- Next steps:
  ...
  - Optionally clean up the local-homeserver live fixtures (users
    ...
    reusable by `e2e/live/cinny133-invite-menu-portal.spec.ts`.
- Review:
...
- Validation:
  ...
  - Live host-surface verification (Vite dev server on `:8090`, local
    ...
    `npx playwright test e2e/live/cinny133-invite-menu-portal.spec.ts`
    ...
    visible, the first option is `Mind, @mindroom_mind:...` (CINNY-132 live
...
### CINNY-132 - Invite autocomplete agent-name ranking and server fallback (2026-07-02)
...
- Risks:
  ...
  - Tier changes affect generic (non-agent) deployments: MXID-server-name-only
    ...
    Cinny ordering.
- Next steps:
  - CINNY-133: portal the suggestion menu so host `overflow` cannot clip it.
- Review:
  - Independent subagent review approved with no blocking defects; it
...
- Status:
  - Complete locally.
- Summary:
  - Removed root-level completion reports that duplicated runbook history:
    `RETROFIT-COMPLETE.md` and `ADAPTER-FIX-COMPLETE.md`.
  - Removed stale rebase/reset scratch docs now superseded by the clean
    v4.12.2 stack and this runbook: `REBASE_PROBLEMS.md` and
    `docs/mindroom-soft-reset-recommit-plan.md`.
  - Removed old paste-attachment agent plan/spec documents under
    `docs/superpowers/`; the implemented behavior and current validation live
    in fork-owned tests plus the relevant runbook entries.
- Decision:
  - Keep durable architecture and playbook docs under `docs/`, but do not keep
    one-off agent execution plans or completion reports as permanent project
    documentation.
- Validation:
  - Green: removed-file smoke check for the deleted transient docs.
  - Green: `npx prettier --check FORK_CHANGES.md`.
  - Green: `git diff --check`.
...
- Status:
  - Superseded by the completed clean-history rewrite and removed in
    CINNY-128.
- Summary:
  ...
  - The plan document was intentionally removed after the rewrite so the repo no
    longer carries stale reset instructions.
- Validation:
...
- Summary:
  ...
  - Follow-up fix: GitHub Actions now derives Android release metadata from the
    MindRoom release calculation, passing `MINDROOM_ANDROID_VERSION_CODE` and
    ...
    changes.
  - Follow-up fix: Gradle validates `MINDROOM_ANDROID_VERSION_CODE` before
    calling `toInteger()`, so malformed CI input fails with a clear error
    instead of an unhelpful conversion exception.
...
- Validation:
  ...
  - Green: `npm run build` (existing Vite runtime-config/source-map/chunk-size
    warnings only).
  - Green: `git diff --check`.
  - Green: Android release-code smoke check:
    `v4.12.2-mindroom.3` -> `4120203`.
  - Green: Gradle configuration check in a JDK 21 container after
    `npx cap sync android`:
    `MINDROOM_ANDROID_VERSION_CODE=4120203 MINDROOM_ANDROID_VERSION_NAME=4.12.2 ./gradlew --no-daemon :app:tasks --all`.
  - Green: malformed-version guard check in a JDK 21 container fails with
    `MINDROOM_ANDROID_VERSION_CODE must be an integer`.
