...
| Room/thread cache and preload | Started. Timeline renderability, room-surface entry derivation, preload counts, eager current-room preload, raw event cache access, cache-order/hydration helper derivation, and thread cache coverage helpers now have seams outside `RoomTimeline`; room/thread cache hydrate and persist orchestration still lives there. | Extract the remaining cache hydrate/persist commands in small behavior-preserving slices. |
...
renderability/preload counting, eager current-room preload, raw event cache access,
cache-order/hydration helper derivation, and thread cache coverage helpers have been moved out of
`RoomTimeline`. The next useful cleanup is extracting the remaining async cache hydrate/persist
orchestration behind controller seams.
