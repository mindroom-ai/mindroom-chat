...
| Filtering, sorting, counts    | Converted at the room index seam. `useMindroomThreadIndex` assembles normal/compact root data, room-surface entries, timeline-derived reply/participant/summary maps, scheduled counts, normal/compact `ThreadRecord` maps, overview ordering, focused-route bypass, effective filters, status counts, tag counts, room-thread list state, and overview cache hydration. Room overview focus/filter helpers now live in `src/app/mindroom/threads/threadRoomFocus.ts`; `RoomTimeline` consumes the snapshot and helper outputs. | Keep new overview/index derivation in `useMindroomThreadIndex` or lower fork-owned selectors.                                           |
...
| Command palette               | Converted at the thread-source seam. Generic command-palette item assembly delegates thread sourcing to `src/app/mindroom/threads/commandPaletteThreadItems.ts`; that owner builds recent-thread and SDK-thread `ThreadRecord` objects, reads thread tags, handles current-thread resolved mutations, merges duplicate thread items, and renders a MindRoom-owned `CommandPaletteThreadViewModel`.                                                                                                                                                                                            | Keep remaining action/user/room/message logic separate; do not reintroduce thread-specific derivation into `features/command-palette`.    |
| Message content rendering     | Converted at the message-render seam. Generic `RenderMessageContent` delegates textual MindRoom policy to `src/app/mindroom/messages/renderMindroomMessageContent.tsx`, which owns summary cards, tool approvals, long-text rendering, AI-run streaming markers, and tool-trace parser options.                                                                                                                                                                                                                              | Keep generic attachment/media rendering in the generic component; do not reintroduce raw MindRoom message metadata decisions there.       |
...
share the `ThreadRecord` seam. Command-palette thread sourcing now lives in
`src/app/mindroom/threads/commandPaletteThreadItems.ts`, so generic palette code no longer reads
thread tag state, recent-thread atoms, or thread-record builders directly. `RoomTimeline` no longer
...
- Thread indicator rendering lives in `src/app/mindroom/threads/ThreadIndicator.tsx`; generic reply
  rendering should only mount the indicator and should not own activity, resolution, scheduled-task,
  unread, participant, or reply-count derivation.
