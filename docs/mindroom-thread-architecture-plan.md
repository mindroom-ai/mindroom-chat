...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Compact room cards            | Partly converted. `CompactThreadCard` renders from `CompactThreadCardViewModel`.                                                                                           | Keep moving selector inputs toward canonical `ThreadRecord` instead of old metadata maps.                          |
| Normal room thread badge      | Mostly converted. `RoomTimeline` now delegates badge count, zero-reply eligibility, participants, summary, preview, and resolved props to `ThreadBadgeViewModel` builders. | Move the source maps behind canonical `ThreadRecord` selectors and make the badge renderer a fork-owned component. |
