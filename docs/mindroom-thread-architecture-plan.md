...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Compact room cards            | Mostly converted. `CompactThreadCard` renders from `CompactThreadCardViewModel`, and that view model now adapts from the shared `ThreadRecord` selector.                   | Stop passing legacy metadata/source maps directly by building a per-room `ThreadRecord` map first. |
| Normal room thread badge      | Mostly converted. `ThreadBadgeViewModel` now adapts from the shared `ThreadRecord` selector for count, zero-reply eligibility, participants, summary, and resolved status. | Make the badge renderer a fork-owned component and feed it records instead of ad-hoc map lookups.  |
| Thread context banner         | Not converted. Header summary/tag/status logic is still independent.                                                                                                       | Add `ThreadHeaderViewModel` plus tag/status mutation seams.                                        |
| Recent threads sidebar        | Not converted. It still uses `useRecentThreadSummary` and its own fallback chain.                                                                                          | Feed sidebar entries from the same thread view-model layer.                                        |
| Command palette               | Not converted. It still derives presentation text and tags independently.                                                                                                  | Add command-palette thread view models backed by the shared selector/index.                        |
...
The immediate next code slice is still Phase 1/2 cleanup, not Phase 5 preload or Phase 6 scroll.
Do not start cache/preload extraction until the summary/status/tag selector seam is proven across
at least compact cards and normal room badges.
