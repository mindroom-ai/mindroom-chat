...
| Room/thread cache and preload | Started. Timeline renderability, room-surface entry derivation, and preload counts live outside `RoomTimeline`; the async preload/cache orchestration still lives there.     | Extract `eventRepository` and `preloadController` in small behavior-preserving slices.        |
...
The immediate next code slice is Phase 5 preload/cache cleanup, not Phase 6 scroll.
...
renderability/preload counting has been moved out of `RoomTimeline`. The next useful cleanup is
extracting the async cache/preload orchestration behind repository/controller seams.
