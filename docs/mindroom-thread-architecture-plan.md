...
| Normal room thread badge      | Mostly converted. `RoomTimeline` now builds a per-room `ThreadRecord` map and badge view models are record-only; legacy direct badge builders are gone.                  | Extract the JSX badge renderer from `RoomTimeline` into a fork-owned component.               |
