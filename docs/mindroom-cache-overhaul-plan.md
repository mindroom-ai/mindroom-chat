...
expected-RED as annotated; streamed-edit's final-tip re-run was blocked
...
| ---- | ------ | --------------------------------- | -------------- | ----------------------- | ---- |
| AC1  | ☐      |                                   |                |                         |      |
| AC2  | ☐ RED (live) — engine converged, render gap remains | Reconciler landed in P5.1 (`engine/reconciler.ts` — `scheduleReconcile({reason:'open-thread-choke-point'\|'room-open'\|'resume'})`). AC2 revision R1-R9 (2026-07-04) replaced the pre-revision per-branch schedule call sites (`open-complete-coverage`, `open-partial-coverage`, `open-backfill-completed`) and STEP 3's guard-abort retry machinery with a single unskippable choke-point schedule at the top of `runThreadOpenCacheFirst`, right after the two hydrate guards and ABOVE every coverage/bootstrap conditional (see FORK_CHANGES.md Runbook "AC2 revision R1-R9" entry). Every thread open that survives the guards schedules exactly one reconcile through the P4.1 scheduler under `kind: 'reconcile'` band 0; every room open does the same with `threadId: undefined`. The reconciler fetches `/relations` (recurse, limit 200), pages further until overlap by event id (removes F7's 200-event ceiling), diffs, and runs the P1.2 `hydrateCachedEvents` machinery to apply missed edits / redactions / aggregation cleanup in place, then fires ONE batched `onRepaired` tick. Every raw event is funneled through `createPreferLiveEventMapper` so Tuwunel's ~10s stale-copy window is healed via the SDK's `Relations.BeforeRedaction` cascade (invariant I2). The reconciler runs guard-free to completion regardless of navigation (component-mount checks live inside `onRepaired`), realizing I2's engine-independent-of-component-navigation clause. AC2 e2e spec `e2e/live/cinny207-stale-cache-divergence.spec.ts` still carries `test.fail()` — the docker gate on the revision tip proves engine convergence in vivo (`threadOpens=2 == threadOpenScheduledCacheFirst=1 + threadOpenSkipCacheFirstPostHydrateGuard=1` invariant sum holds; `reconcilesScheduled=2, reconcilesRepaired=1, reconcilerPersists=1, reconcilesOnRepairedFired=1, reconcilesThreadNull=0`) but the visible `edit-target v2 converged` assertion still times out (`element(s) not found`). The gap is downstream of the engine — in the render pipeline consuming `fallbackThreadEventsState.events` and `thread.events` — with three candidates named in the spec header and Runbook (mergeThreadRenderEvents dedup keeping first-instance seen; React batching/memo swallowing the tick; SDK inject leg running against a live thread whose bootstrap was skipped). Unit layer green (2644/2644 vitest, tsc clean, 18/0 lint, build clean); regression on `cinny207-streamed-edit-cache` and `cinny207-stop-emoji-redaction` green on the revision tip. | stale edit/reaction/redaction persist on cache-hit reopen (I2/AC2 pre-P5 — no revalidation path) → cache converges to server truth in place after every open |                         |      |
