...
HUMAN_TOK=$(curl -s -X POST $HS/_matrix/client/v3/login -d '{"type":"m.login.password","identifier":{"type":"m.id.user","user":"invite-tester"},"password":"invite-pw-2026"}' | python3 -c 'import json,sys;print(json.load(sys.stdin)["access_token"])')
AGENT_TOK=$(curl -s -X POST $HS/_matrix/client/v3/login -d '{"type":"m.login.password","identifier":{"type":"m.id.user","user":"mindroom_sarro"},"password":"agent-pw-2026"}' | python3 -c 'import json,sys;print(json.load(sys.stdin)["access_token"])')
